...
namespace DAO
{
    public class Nha_Cung_CapDAO
    {
        ...
        public List<string> GetValuesForAutoComplete(string filterValue, string columnName)
        {
            using (var context = new MyDbContext())
            {
                var distinctValues = new List<string>();
                distinctValues.AddRange(context.Database.SqlQuery<string>(
                    $"SELECT DISTINCT CAST({columnName} AS NVARCHAR(255)) AS {columnName} FROM NHACUNGCAP WHERE MaNCC = @p0 OR TenNCC = @p0", filterValue));
                distinctValues.AddRange(context.Database.SqlQuery<string>(
                    $"SELECT DISTINCT CAST({columnName} AS NVARCHAR(255)) AS {columnName} FROM SANPHAM WHERE TenSP = @p0", filterValue));
